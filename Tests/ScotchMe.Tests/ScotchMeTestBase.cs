...
using ScotchMe.Migrations.SeedData;
...
namespace ScotchMe.Tests
{
    public abstract class ScotchMeTestBase : AbpIntegratedTestBase<ScotchMeTestModule>
    {
        ...
        protected void UsingDbContext(Action<ScotchMeDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ScotchMeDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ScotchMeDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ScotchMeDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ScotchMeDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ScotchMeDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ScotchMeDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ScotchMeDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ScotchMeDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ScotchMeDbContext, Task<T>> func)
        {
            T result;
